...
silêncio. Foi exatamente o que aconteceu entre 22/07/2026 e 24/07/2026 (e de novo
em 16/09/2026, com 12 colisões de uid — este repo deployando cópias em subpastas
de dashboards cuja fonte de verdade é o repo eddie-auto-dev, na raiz do provider).
...
Modos de uso:
  ...
  --repo dashboards --provider-dir /home/.../provisioning/dashboards
      ...
      ATENÇÃO: o path do provider é compartilhado com o repo eddie-auto-dev.
      A checagem de estado projetado inclui o que já está no servidor — ou seja,
      se o servidor ainda tiver duplicatas em disco, o deploy aborta até que a
      limpeza aconteça. Esta é a proteção pretendida.

      Ignora o diretório de quarentena (dashboard_quarantine/) e backups, que
      nunca podem ficar dentro do path do provider.
...
def iter_dashboards(root: Path, exclude: tuple[str, ...] = ()) -> list[Path]:
    """Lista os .json que o Grafana enxergaria sob `root`, exceto paths com `exclude`."""
    found: list[Path] = []
    if not root.is_dir():
        return found
...
def projected_layout(repo_dir: Path, provider_dir: Path | None) -> dict[str, Path]:
    ...
    if provider_dir is not None:
        for existing in iter_dashboards(provider_dir, exclude=("dashboard_quarantine",)):
